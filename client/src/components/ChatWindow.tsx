        ...
        (messageObj.msgType === "msg") ?
          <React.Fragment key={i}>
            <span>
              ...
              {(messageObj.auth === "admin") ? "º" :(messageObj.auth === "user")?  "*": ""}<a href={messageObj.pageUrl}>{messageObj.name}</a>{(messageObj.iconUrl !== '') ? <> <img src={'/src/assets/icons/' + messageObj.iconUrl + '.gif'} /></> : ''}: <span style={{color: messageObj.color}}>{messageObj.message}</span>
            </span>
            <br />
          </React.Fragment>
